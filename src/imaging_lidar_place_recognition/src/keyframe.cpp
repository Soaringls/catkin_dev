...
void keypointConverter(vector<cv::KeyPoint> &keypoints_in,
                       vector<cv::Point2f> &points_in) {
...
void keypointConverter(vector<cv::Point2f> &points_in,
...
void pubMatchedImages(const ros::Publisher *this_pub, const double time_stamp,
                      const bool draw_lines, const int circle_size,
                      const cv::Scalar line_color, const int index_new,
                      const int index_old, const cv::Mat &image_new,
                      const cv::Mat &image_old,
                      const vector<cv::Point2f> &features_new,
                      const vector<cv::Point2f> &features_old) {
  if (this_pub->getNumSubscribers() == 0) return;

  int gap = 1;
  cv::Mat gap_image(gap, image_new.size().width, CV_8UC1,
  ...
  for (int i = 0; i < (int)features_new.size(); i++) {
    ...
    cv::circle(color_img, cur_pt, circle_size * MATCH_IMAGE_SCALE,
               cv::Scalar /*bgr yellow*/ (0, 255, 255) /*line_color*/,
               MATCH_IMAGE_SCALE * 2);
  }
  ...
  for (int i = 0; i < (int)features_old.size(); i++) {
    ...
    cv::circle(color_img, old_pt, circle_size * MATCH_IMAGE_SCALE,
               /*line_color*/ cv::Scalar /*bgr*/ (255, 0, 0) /*red*/,
               MATCH_IMAGE_SCALE * 2);
  }
  ...
  if (draw_lines) {
    for (int i = 0; i < (int)features_new.size(); i++) {
      cv::Point2f old_pt = features_old[i] * MATCH_IMAGE_SCALE;
      old_pt.y += image_new.size().height + gap;
      cv::line(color_img, features_new[i] * MATCH_IMAGE_SCALE, old_pt,
               line_color, MATCH_IMAGE_SCALE * 2, 8, 0);
    }
  }
  // plot text
  ...
  //             CV_FONT_HERSHEY_SIMPLEX,  text_scale, text_color,
  //             text_thickness);
  // cv::putText(color_img, "Frame: " + to_string(index_old),
  //             cv::Point2f(5, text_height_pos + image_new.size().height +
  //             gap), CV_FONT_HERSHEY_SIMPLEX,  text_scale, text_color,
  //             text_thickness);
  // publish matched image
  sensor_msgs::ImagePtr msg =
      cv_bridge::CvImage(std_msgs::Header(), "bgr8", color_img).toImageMsg();
...
void KeyFrame::computeWindowOrbPoint() {
  if (USE_ORB) {
    ...
    detector->compute(image_intensity, orb_window_keypoints,
                      orb_window_descriptors);
  }
}
...
void KeyFrame::computeWindowBriefPoint() {
  if (USE_BRIEF) {
    // Corner feautres
    vector<uchar> status;
    cv::goodFeaturesToTrack(image_intensity, brief_point_2d_uv,
                            NUM_BRI_FEATURES, 0.01, 10, MASK);
    extractPoints(brief_point_2d_uv, brief_point_3d, brief_point_2d_norm,
                  status);
    ...
    briefExtractor(image_intensity, brief_window_keypoints,
                   brief_window_descriptors);
  }
}
...
void KeyFrame::computeSearchOrbPoint() {
  if (USE_ORB) {
    ...
    cv::Ptr<cv::ORB> detector =
        cv::ORB::create(NUM_ORB_FEATURES * 5, 1.2f, 8, 5);
    detector->detect(image_intensity, search_orb_keypoints, MASK);
    keypointConverter(search_orb_keypoints, search_orb_point_2d_uv);
    extractPoints(search_orb_point_2d_uv, search_orb_point_3d,
                  search_orb_point_2d_norm, status);
    ...
    detector->compute(image_intensity, search_orb_keypoints,
                      search_orb_descriptors);
  }
}
...
void KeyFrame::computeSearchBriefPoint() {
  if (USE_BRIEF) {
    // Corner feautres
    vector<uchar> status;
    cv::goodFeaturesToTrack(image_intensity, search_brief_point_2d_uv,
                            NUM_BRI_FEATURES * 5, 0.01, 2, MASK);
    extractPoints(search_brief_point_2d_uv, search_brief_point_3d,
                  search_brief_point_2d_norm, status);
    ...
    briefExtractor(image_intensity, search_brief_keypoints,
                   search_brief_descriptors);
  }
}
...
void KeyFrame::computeBoWPoint() {
  ...
  if (DEBUG_IMAGE) {
    cv::Mat imgShow;
    cv::drawKeypoints(image_intensity, keypoints, imgShow, cv::Scalar::all(-1),
...
bool KeyFrame::findConnection(KeyFrame *old_kf) {
  ...
  pubMatchedImages(&pub_bow_img, time_stamp, false, 1, cv::Scalar(0, 255, 0),
                   index, old_kf->index, thumbnail, old_kf->thumbnail,
                   search_orb_point_2d_uv, old_kf->search_orb_point_2d_uv);
  ...
  if (USE_ORB) {
    ...
    matcher.match(orb_window_descriptors, old_kf->search_orb_descriptors,
                  matches);

    std::sort(matches.begin(), matches.end());
    for (size_t i = 0; i < matches.size(); ++i) {
      good_matches.push_back(matches[i]);
      if (matches[i].distance > matches[0].distance * 2) break;
    }
    ...
    if ((int)good_matches.size() > MIN_LOOP_FEATURE_NUM) {
      ...
      for (size_t i = 0; i < good_matches.size(); i++) {
        ...
        matched_2d_old_norm.push_back(
            old_kf->search_orb_point_2d_norm[old_index]);
      }

      pubMatchedImages(&pub_prepnp_img, time_stamp, true, 5,
                       cv::Scalar(0, 255, 0), index, old_kf->index, thumbnail,
      ...
      if ((int)matched_2d_cur.size() > MIN_LOOP_FEATURE_NUM &&
        ...
        pubMatchedImages(&pub_match_img, time_stamp, true, 5,
                         cv::Scalar(0, 255, 0), index, old_kf->index, thumbnail,
                         old_kf->thumbnail, matched_2d_cur, matched_2d_old);
        return true;
      }
  ...
  if (USE_BRIEF) {
    ...
    searchByBRIEFDes(matched_2d_old, matched_2d_old_norm, status,
                     brief_window_descriptors, old_kf->search_brief_descriptors,
                     old_kf->search_brief_point_2d_uv,
...
void KeyFrame::PnPRANSAC(const vector<cv::Point2f> &matched_2d_old_norm,
  ...
  solvePnPRansac(matched_3d, matched_2d_old_norm, K, D, rvec, tvec, false, 100,
                 0.025, 0.99, inliers);
...
#pragma omp parallel for num_threads(NUM_THREADS)
  for (size_t i = 0; i < descriptors_now.size(); i++) {
    ...
    if (searchInAera(descriptors_now[i], descriptors_old, keypoints_old,
...
bool KeyFrame::distributionValidation(
    const vector<cv::Point2f> &new_point_2d_uv,
    const vector<cv::Point2f> &old_point_2d_uv) {
  if (new_point_2d_uv.empty()) return false;
  ...
  pcl::PointCloud<pcl::PointXYZ>::Ptr old_cloud(
      new pcl::PointCloud<pcl::PointXYZ>());
  ...
  pcl::computeCovarianceMatrix(*new_cloud, new_xyz_centroid,
  ...
  pcl::computeCovarianceMatrix(*old_cloud, old_xyz_centroid,
                               old_covariance_matrix);
  ...
  if (cov_x_diff > 3 * std::min(new_cov_x, old_cov_x) ||
      cov_y_diff > 0.75 * std::min(new_cov_y, old_cov_y)) {
    return false;
  }
