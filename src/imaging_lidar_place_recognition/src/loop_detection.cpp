...
void LoopDetector::addKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop) {
  ...
  // add keyframe
  cur_kf->freeMemory();
  keyframe_list.push_back(cur_kf);
}

KeyFrame* LoopDetector::getKeyFrame(int index) {
  list<KeyFrame*>::iterator it = keyframe_list.begin();
  for (; it != keyframe_list.end(); it++) {
    if ((*it)->index == index) break;
  }
  if (it != keyframe_list.end())
    return *it;
  else
    return NULL;
}
...
int LoopDetector::detectLoop(KeyFrame* keyframe, int frame_index) {
  ...
  db.query(keyframe->bow_descriptors, ret, 4,
           frame_index - MIN_LOOP_SEARCH_GAP);
  db.add(
      keyframe->bow_descriptors);  // ret[0] is the nearest neighbour's score.
                                   // threshold change with neighour score
  ...
  if (DEBUG_IMAGE) {
    ...
    if (ret.size() > 0)
      putText(bow_images, "Index: " + to_string(frame_index),
              cv::Point2f(10, 50), CV_FONT_HERSHEY_SIMPLEX, 1.0,
              cv::Scalar(255), 2);
    ...
    for (unsigned int i = 0; i < ret.size(); i++) {
      ...
      putText(tmp_image,
              "Index:  " + to_string(tmp_index) +
                  ", BoW score:" + to_string(ret[i].Score),
              cv::Point2f(10, 50), CV_FONT_HERSHEY_SIMPLEX, 1.0,
              cv::Scalar(255), 2);
  ...
  if (frame_index - MIN_LOOP_SEARCH_GAP < 0) return -1;
