...
class ImageHandler {
 public:
  ...
  ImageHandler() {
    ...
    pub_image =
        nh.advertise<sensor_msgs::Image>("loop_detector/image_stack", 1);
  }

  void cloud_handler(const sensor_msgs::PointCloud2ConstPtr& cloud_msg) {
    // convert cloud
    pcl::PointCloud<PointOuster>::Ptr laser_cloud(
        new pcl::PointCloud<PointOuster>());
    pcl::fromROSMsg(*cloud_msg, *laser_cloud);

    // IMAGE_HEIGHT * IMAGE_WIDTH config by params of lidar
    ...
    image_intensity =
        cv::Mat(IMAGE_HEIGHT, IMAGE_WIDTH, CV_8UC1, cv::Scalar(0));
...
#pragma omp parallel for num_threads(NUM_THREADS)
    ...
    if (pub_image.getNumSubscribers() != 0) {
      ...
      cv::cvtColor(image_visualization, image_visualization, CV_GRAY2RGB);
      cv::putText(image_visualization, "Ambient",
                  cv::Point2f(5, 20 + IMAGE_HEIGHT * 0),
                  CV_FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(255, 0, 255), 2);
      cv::putText(image_visualization, "Intensity",
                  cv::Point2f(5, 20 + IMAGE_HEIGHT * 1),
                  CV_FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(255, 0, 255), 2);
      cv::putText(image_visualization, "Range",
                  cv::Point2f(5, 20 + IMAGE_HEIGHT * 2),
                  CV_FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(255, 0, 255), 2);
      pubImage(&pub_image, image_visualization, cloud_msg->header, "bgr8");
    }
    ...
    static tf::Transform base_to_lidar = tf::Transform(
        tf::createQuaternionFromRPY(0, 0, 0), tf::Vector3(0, 0, 0));
    tf_base_to_lidar.sendTransform(tf::StampedTransform(
        base_to_lidar, cloud_msg->header.stamp, "base_link", "velodyne"));
  }

  void pubImage(ros::Publisher* this_pub, const cv::Mat& this_image,
                std_msgs::Header this_header, string image_format) {
