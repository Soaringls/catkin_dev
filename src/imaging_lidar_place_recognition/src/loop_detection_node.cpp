...
void visualizeLoopClosure(ros::Publisher *pub_m, ros::Publisher *pub_i,
  ...
  if (cloud_path->empty() || match_container.empty()) return;
...
void cloud_handler(const sensor_msgs::PointCloud2ConstPtr &cloud_msg) {
  ...
  KeyFrame *keyframe =
      new KeyFrame(cloud_time, global_frame_index,
                   image_handler->image_intensity, image_handler->cloud_track);

  // detect loop
  loopDetector.addKeyFrame(keyframe, 1);

  // visualize loop
  index_poseindex_container[global_frame_index] =
      std::max((int)cloud_traj->size() - 1, 0);
  visualizeLoopClosure(&pub_marker, &pub_index, cloud_msg->header.stamp,
                       index_match_container, index_poseindex_container,
                       cloud_traj);
...
int main(int argc, char **argv) {
  ...
  pub_match_msg =
      n.advertise<std_msgs::Float64MultiArray>("loop_detector/time", 1);
  ...
  pub_marker =
      n.advertise<visualization_msgs::MarkerArray>("loop_detector/marker", 1);
